...
func (s dbOutboxStore) RecordPublished(id string) error {
	_, err := s.db.Execute(`INSERT INTO outbox_publications (outbox_event_id, published_at)
SELECT $1, now() WHERE NOT EXISTS (SELECT 1 FROM outbox_publications WHERE outbox_event_id = $1)`, id)
	return err
}

func (s dbOutboxStore) RecordFailure(id string, reason string) error {
	_, err := s.db.Execute(`INSERT INTO outbox_publication_failures (id, outbox_event_id, reason, failed_at)
